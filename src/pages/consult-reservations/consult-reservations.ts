  ...
  constructor(public navCtrl: NavController, 
    ...
    confirmReportIssues( meetingRoom:any ) {
      ...
      let subs = this.events_manager
                     ...
                     .subscribe( msg => {
                      this.reportIssues( meetingRoom,msg );
                      subs.unsubscribe();
    ...
    reportIssues(meetingRoom,msg){
      ...
      this.meeting_provider.sendIncident( meetingDetail ).subscribe( response => {
        this.events_manager.setGeneralNotificationMessage('El reporte se envió correctamente','success');
      }, error => {
        this.events_manager.setGeneralNotificationMessage('El reporte no se envió correctamente','error','top');
      });
    }
    ...
    onInput() {
      if(this.inputSearchBar.trim() != ''){
        this.meetings = this.meetings.filter( (meetingRoom:any) => {
          return (meetingRoom.meetingRoomName.toLowerCase().indexOf(this.inputSearchBar.toLowerCase()) > -1);
        });
